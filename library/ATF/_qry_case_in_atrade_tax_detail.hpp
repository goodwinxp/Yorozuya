// This file auto generated by plugin for ida pro. Generated code only for x64. Please, dont change manually
#pragma once

#include "./common/common.h"
#include "_qry_case_in_atrade_tax_info.hpp"


START_ATF_NAMESPACE
    namespace detail
    {
        static info::_qry_case_in_atrade_taxctor__qry_case_in_atrade_tax2_ptr _qry_case_in_atrade_taxctor__qry_case_in_atrade_tax2_next(nullptr);
        static info::_qry_case_in_atrade_taxctor__qry_case_in_atrade_tax2_clbk _qry_case_in_atrade_taxctor__qry_case_in_atrade_tax2_user(nullptr);
        static info::_qry_case_in_atrade_taxsize4_ptr _qry_case_in_atrade_taxsize4_next(nullptr);
        static info::_qry_case_in_atrade_taxsize4_clbk _qry_case_in_atrade_taxsize4_user(nullptr);
        
        static void _qry_case_in_atrade_taxctor__qry_case_in_atrade_tax2_wrapper(struct _qry_case_in_atrade_tax* _this)
        {
           _qry_case_in_atrade_taxctor__qry_case_in_atrade_tax2_user(_this, _qry_case_in_atrade_taxctor__qry_case_in_atrade_tax2_next);
        };
        static int _qry_case_in_atrade_taxsize4_wrapper(struct _qry_case_in_atrade_tax* _this)
        {
           return _qry_case_in_atrade_taxsize4_user(_this, _qry_case_in_atrade_taxsize4_next);
        };
        
        static hook_record _qry_case_in_atrade_tax_functions[] = {
        {   (LPVOID)0x1402605a0L,
            (LPVOID *)&_qry_case_in_atrade_taxctor__qry_case_in_atrade_tax2_user,
            (LPVOID *)&_qry_case_in_atrade_taxctor__qry_case_in_atrade_tax2_next,
            (LPVOID)cast_pointer_function(_qry_case_in_atrade_taxctor__qry_case_in_atrade_tax2_wrapper),
            (LPVOID)cast_pointer_function((void(_qry_case_in_atrade_tax::*)())&_qry_case_in_atrade_tax::ctor__qry_case_in_atrade_tax) },
        {   (LPVOID)0x140260620L,
            (LPVOID *)&_qry_case_in_atrade_taxsize4_user,
            (LPVOID *)&_qry_case_in_atrade_taxsize4_next,
            (LPVOID)cast_pointer_function(_qry_case_in_atrade_taxsize4_wrapper),
            (LPVOID)cast_pointer_function((int(_qry_case_in_atrade_tax::*)())&_qry_case_in_atrade_tax::size) },
        
        };
        
    }; // static end namespace detail
END_ATF_NAMESPACE
