// This file auto generated by plugin for ida pro. Generated code only for x64. Please, dont change manually
#pragma once

#include "./common/common.h"
#include "_nuclear_bomb_explosion_result_zocl_info.hpp"


START_ATF_NAMESPACE
    namespace detail
    {
        static info::_nuclear_bomb_explosion_result_zoclctor__nuclear_bomb_explosion_result_zocl2_ptr _nuclear_bomb_explosion_result_zoclctor__nuclear_bomb_explosion_result_zocl2_next(nullptr);
        static info::_nuclear_bomb_explosion_result_zoclctor__nuclear_bomb_explosion_result_zocl2_clbk _nuclear_bomb_explosion_result_zoclctor__nuclear_bomb_explosion_result_zocl2_user(nullptr);
        static info::_nuclear_bomb_explosion_result_zoclsize4_ptr _nuclear_bomb_explosion_result_zoclsize4_next(nullptr);
        static info::_nuclear_bomb_explosion_result_zoclsize4_clbk _nuclear_bomb_explosion_result_zoclsize4_user(nullptr);
        
        static void _nuclear_bomb_explosion_result_zoclctor__nuclear_bomb_explosion_result_zocl2_wrapper(struct _nuclear_bomb_explosion_result_zocl* _this)
        {
           _nuclear_bomb_explosion_result_zoclctor__nuclear_bomb_explosion_result_zocl2_user(_this, _nuclear_bomb_explosion_result_zoclctor__nuclear_bomb_explosion_result_zocl2_next);
        };
        static int _nuclear_bomb_explosion_result_zoclsize4_wrapper(struct _nuclear_bomb_explosion_result_zocl* _this)
        {
           return _nuclear_bomb_explosion_result_zoclsize4_user(_this, _nuclear_bomb_explosion_result_zoclsize4_next);
        };
        
        static hook_record _nuclear_bomb_explosion_result_zocl_functions[] = {
        {   (LPVOID)0x14013e680L,
            (LPVOID *)&_nuclear_bomb_explosion_result_zoclctor__nuclear_bomb_explosion_result_zocl2_user,
            (LPVOID *)&_nuclear_bomb_explosion_result_zoclctor__nuclear_bomb_explosion_result_zocl2_next,
            (LPVOID)cast_pointer_function(_nuclear_bomb_explosion_result_zoclctor__nuclear_bomb_explosion_result_zocl2_wrapper),
            (LPVOID)cast_pointer_function((void(_nuclear_bomb_explosion_result_zocl::*)())&_nuclear_bomb_explosion_result_zocl::ctor__nuclear_bomb_explosion_result_zocl) },
        {   (LPVOID)0x14013e6d0L,
            (LPVOID *)&_nuclear_bomb_explosion_result_zoclsize4_user,
            (LPVOID *)&_nuclear_bomb_explosion_result_zoclsize4_next,
            (LPVOID)cast_pointer_function(_nuclear_bomb_explosion_result_zoclsize4_wrapper),
            (LPVOID)cast_pointer_function((int(_nuclear_bomb_explosion_result_zocl::*)())&_nuclear_bomb_explosion_result_zocl::size) },
        
        };
        
    }; // static end namespace detail
END_ATF_NAMESPACE
